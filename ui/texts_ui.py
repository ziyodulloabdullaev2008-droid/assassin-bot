...
def build_texts_keyboard(
    texts: list, back_callback: str = "bc_text"
    ...
    for i in range(len(texts)):
        text_buttons.append(
            InlineKeyboardButton(text=f"Text {i + 1}", callback_data=f"text_view_{i}")
        )
...
def build_text_settings_keyboard(
    text_mode: str = "random", parse_mode: str = "HTML"
) -> InlineKeyboardMarkup:
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                text=f"Формат: {parse_mode}", callback_data="text_format_toggle"
            )
        ],
