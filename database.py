...
def save_broadcast_config(user_id: int, config: dict):
    """Сохранить конфиг рассылки в JSON с резервной копией предыдущей версии."""
    ...
    try:
        # Сначала сохраняем резервную копию текущего конфига
        if config_file.exists():
            try:
                shutil.copy2(config_file, backup_file)
            except Exception as e:
                print(f"Ошибка создания backup конфига для {user_id}: {e}")

        # Затем записываем новый конфиг в основной файл
        with open(config_file, "w", encoding="utf-8") as f:
            json.dump(config, f, ensure_ascii=False, indent=2)

    except Exception as e:
        print(f"Ошибка сохранения конфига рассылки для {user_id}: {e}")
        # Если запись не удалась, пробуем восстановить файл из backup
        if backup_file.exists():
            try:
                shutil.copy2(backup_file, config_file)
                print(f"Backup восстановлен для пользователя {user_id}")
            except Exception as restore_error:
                print(f"Ошибка восстановления backup: {restore_error}")


def load_broadcast_config(user_id: int) -> dict:
    """Загрузить конфиг рассылки из JSON с fallback на backup."""

    user_broadcast_dir(user_id)
    config_file = broadcast_config_path(user_id)
    backup_file = broadcast_config_backup_path(user_id)

    # Переносим старые конфиги в новую структуру путей
    ...
    if config_file.exists():
        try:
            ...
            print(f"Ошибка чтения конфига рассылки для {user_id}: {e}")
            # Пробуем загрузить резервную копию
            if backup_file.exists():
                try:
                    with open(backup_file, "r", encoding="utf-8") as f:
                        config = json.load(f)
                    print(f"Загружен backup конфиг для пользователя {user_id}")
                    return config
                except Exception as backup_error:
                    print(f"Ошибка чтения backup конфига: {backup_error}")

    # Возвращаем дефолтный конфиг, если ничего не загрузилось
...
def add_vip_user(user_id: int, days: int = 0) -> bool:
    ...
    try:
        ...
        print(f"Ошибка при добавлении VIP пользователя: {str(e)}")
        return False
...
def remove_vip_user(user_id: int) -> bool:
    ...
    try:
        ...
        print(f"Ошибка при удалении VIP пользователя: {str(e)}")
        return False
...
def get_all_vip_users() -> List[int]:
    ...
    try:
        ...
        print(f"Ошибка при получении списка VIP пользователей: {str(e)}")
        return []
