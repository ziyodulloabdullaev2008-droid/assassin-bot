...
def get_broadcast_config(user_id: int) -> dict:
    ...
    if "texts" not in config:
        config["texts"] = []
    if "text_source_type" not in config:
        config["text_source_type"] = "manual"
    if "text_mode" not in config:
        config["text_mode"] = "random"
    if "text_index" not in config:
        config["text_index"] = 0
    if "source_channel_ref" not in config:
        config["source_channel_ref"] = ""
    if "source_channel_title" not in config:
